...
public interface IWorkFlowRepository extends JpaRepository<WorkFlow, BigInteger> {
    ...
    @Query(value = "Select * from work_flow wf where wf.hp_profile_id = :hpProfileId and wf.application_type_id IN (3,4) and wf.work_flow_status_id = 2 order by created_at desc limit 1", nativeQuery = true)
