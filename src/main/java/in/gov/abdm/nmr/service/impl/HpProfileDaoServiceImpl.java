...
import in.gov.abdm.nmr.repository.ICustomQualificationDetailRepository;
...
public class HpProfileDaoServiceImpl implements IHpProfileDaoService {
	...
	public HpProfileDaoServiceImpl(IHpProfileMapper ihHpProfileMapper, IHpProfileRepository iHpProfileRepository,
		...
		this.iCustomQualificationDetailRepository = iCustomQualificationDetailRepository;
		this.hpNbeDetailsRepository = hpNbeDetailsRepository;
	}
...
//		List<Predicate> predicates = new ArrayList<>();
		...
		if (hpProfileCheck.getTransactionId() != null && (hpProfileCheck.getESignStatus() == null
				|| hpProfileCheck.getESignStatus().equalsIgnoreCase(NMRConstants.E_SIGN_FAILURE_STATUS))) {
...
//			currentAddressTO.setUpdatedDate(currentAddress.get("updated_date", String.class));
			...
			if (iHpProfileRepository.existsById(hpProfileId) == true) {
				...
				if (hpProfileUpdateRequest.getCommunicationAddress() != null) {
					...
					if (addressData != null) {
						...
						addressData.setAddressLine1(hpProfileUpdateRequest.getCommunicationAddress().getAddressLine1());
						...
						communicationState.setId(hpProfileUpdateRequest.getCommunicationAddress().getState().getId());
						...
						addressType.setId(hpProfileUpdateRequest.getCommunicationAddress().getAddressType().getId());
						...
						addAddressData
								.setAddressLine1(hpProfileUpdateRequest.getCommunicationAddress().getAddressLine1());
				...
				if (workProfile == null) {
					...
					addWorkProfile
							.setWorkOrganization(hpProfileUpdateRequest.getCurrentWorkDetails().getWorkOrganization());
				...
				if (registrationDetails == null) {
					...
					addRegistrationDetail
							.setRegistrationDate(hpProfileUpdateRequest.getRegistrationDetail().getRegistrationDate());
					addRegistrationDetail
					...
					registrationDetails.setIsRenewable(hpProfileUpdateRequest.getRegistrationDetail().getIsRenewable());
				...
				if (qualificationDetailList.size() == 0) {
					...
					for (QualificationDetails qualification : qualificationDetailList) {
						...
						qualification.setCountryId(
								hpProfileUpdateRequest.getQualificationDetail().get(cnt).getCountry().getId() == null
										? null
						...
						qualification.setStateId(
								hpProfileUpdateRequest.getQualificationDetail().get(cnt).getState().getId() == null
										? null
										: hpProfileUpdateRequest.getQualificationDetail().get(cnt).getState().getId());

						qualification.setCollegeId(
								hpProfileUpdateRequest.getQualificationDetail().get(cnt).getCollege().getId() == null
										? null
										: hpProfileUpdateRequest.getQualificationDetail().get(cnt).getCollege()
												.getId());

						qualification.setUniversityId(
								hpProfileUpdateRequest.getQualificationDetail().get(cnt).getUniversity().getId() == null
										? null
						...
						qualification.setCourseId(
								hpProfileUpdateRequest.getQualificationDetail().get(cnt).getCourse().getId() == null
										? null
										: hpProfileUpdateRequest.getQualificationDetail().get(cnt).getCourse().getId());
				...
				if (newAddress != null) {
					Address workAddressData = iAddressRepository.getCommunicationAddressByHpProfileId(hpProfileId, 2);
				...
				HpProfileUpdateResponseTO hpProfileUpdateResponseTO = new HpProfileUpdateResponseTO(204,
						"Record Updated Successfully!", hpProfileId, hpProfile.getRequestId());
	...
	@Override
	public void saveQualificationDetails(HpProfile hpProfile, RegistrationDetails newRegistrationDetails,
			List<QualificationDetailRequestTO> qualificationDetailRequestTOS) {
		if (qualificationDetailRequestTOS.size() > 0) {
			List<QualificationDetails> qualificationDetails = qualificationDetailRequestTOS.stream()
					.map(qualificationDetailRequestTO -> {
						QualificationDetails qualification = new QualificationDetails();
						...
						qualification.setRequestId(
								coalesce(qualificationDetailRequestTO.getRequestId(), hpProfile.getRequestId()));
	...
	public HpProfileUpdateResponseTO updateHpPersonalDetails(BigInteger hpProfileId,
		...
		HpProfile addHpProfile = new HpProfile();
		if (iHpProfileRepository.existsById(hpProfileId) == true) {
			...
			hpProfile.setFullName(hpPersonalUpdateRequestTO.getCommunicationAddress().getFullName());
			addHpProfile = iHpProfileRepository.saveAndFlush(hpProfile);

		}

		hpProfileId = addHpProfile.getId() != null ? addHpProfile.getId() : hpProfileId;
...
//					AddressType addressType = iAddressTypeRepository.findById(hpPersonalUpdateRequestTO.getCommunicationAddress().getAddressType().getId()).orElse(null);
				AddressType addressType = new AddressType();
				...
				Address addAddressData = new Address();
				addAddressData.setAddressLine1(hpPersonalUpdateRequestTO.getCommunicationAddress().getAddressLine1());
				addAddressData.setPincode(hpPersonalUpdateRequestTO.getCommunicationAddress().getPincode());
				addAddressData.setEmail(hpPersonalUpdateRequestTO.getCommunicationAddress().getEmail());
				addAddressData.setMobile(hpPersonalUpdateRequestTO.getCommunicationAddress().getMobile());
...
//					AddressType addressType = iAddressTypeRepository.findById(hpPersonalUpdateRequestTO.getCommunicationAddress().getAddressType().getId()).orElse(null);
		...
		HpProfileUpdateResponseTO hpProfileUpdateResponseTO = new HpProfileUpdateResponseTO(204,
				"Record Added/Updated Successfully!", hpProfileId, addHpProfile.getRequestId());
	...
	public HpProfileUpdateResponseTO updateHpRegistrationDetails(BigInteger hpProfileId,
		...
		RegistrationDetails newRegistrationDetail = new RegistrationDetails();

		RegistrationDetails registrationDetail = registrationDetailRepository
				.getRegistrationDetailsByHpProfileId(hpProfileId);

		if (registrationDetail != null) {
			registrationDetail
					.setRegistrationDate(hpRegistrationUpdateRequestTO.getRegistrationDetail().getRegistrationDate());
			registrationDetail
					.setRegistrationNo(hpRegistrationUpdateRequestTO.getRegistrationDetail().getRegistrationNumber());

			StateMedicalCouncil stateMedicalCouncil = iStateMedicalCouncilRepository
					.findById(hpRegistrationUpdateRequestTO.getRegistrationDetail().getStateMedicalCouncil().getId())
					.orElse(null);
			if (stateMedicalCouncil != null) {
				registrationDetail.setStateMedicalCouncil(stateMedicalCouncil);
			}

			registrationDetail.setQualificationFrom(hpRegistrationUpdateRequestTO.getQualificationFrom());
			...
			registrationDetailRepository.saveAndFlush(registrationDetail);
			...
			addRegistrationDetail.setQualificationFrom(hpRegistrationUpdateRequestTO.getQualificationFrom());
			addRegistrationDetail
					.setIsRenewable(hpRegistrationUpdateRequestTO.getRegistrationDetail().getIsRenewable());

			addRegistrationDetail.setRenewableRegistrationDate(
					hpRegistrationUpdateRequestTO.getRegistrationDetail().getRenewableRegistrationDate());

			addRegistrationDetail
					.setIsNameChange(hpRegistrationUpdateRequestTO.getRegistrationDetail().getIsNameChange());

			addRegistrationDetail.setCreatedAt(Timestamp.valueOf(LocalDateTime.now()));
			addRegistrationDetail.setUpdatedAt(Timestamp.valueOf(LocalDateTime.now()));

			HpProfile hpProfile = iHpProfileRepository.findById(hpProfileId).orElse(null);
			addRegistrationDetail.setHpProfileId(hpProfile);
			addRegistrationDetail.setRequestId(hpRegistrationUpdateRequestTO.getRequestId());

			newRegistrationDetail = registrationDetailRepository.saveAndFlush(addRegistrationDetail);
		}

		RegistrationDetails registrationData = newRegistrationDetail == null ? registrationDetail
				: newRegistrationDetail;
		String requestId = hpRegistrationUpdateRequestTO.getRequestId() == null ? registrationData.getRequestId()
				: hpRegistrationUpdateRequestTO.getRequestId();

		List<QualificationDetailRequestTO> newqualificationDetailTOList = hpRegistrationUpdateRequestTO
				.getQualificationDetail();

		if (hpRegistrationUpdateRequestTO.getQualificationFrom().equals("India")) {

			List<QualificationDetails> qualificationDetailList = qualificationDetailRepository
					.getQualificationDetailsByHpProfileId(hpProfileId);
			...
			if (qualificationDetailList.size() == 0) {
				...
				for (QualificationDetailRequestTO newQualification : newqualificationDetailTOList) {
					...
					qualification.setQualificationMonth(newQualification.getQualificationMonth());
					qualification.setIsNameChange(newQualification.getIsNameChange());
					qualification.setRegistrationDetails(registrationData);
				...
				for (QualificationDetails qualification : qualificationDetailList) {
					...
					qualification.setCountryId(
							hpRegistrationUpdateRequestTO.getQualificationDetail().get(cnt).getCountry().getId() == null
									? null
									: hpRegistrationUpdateRequestTO.getQualificationDetail().get(cnt).getCountry()
											.getId());

					qualification.setStateId(
							hpRegistrationUpdateRequestTO.getQualificationDetail().get(cnt).getState().getId() == null
									? null
									: hpRegistrationUpdateRequestTO.getQualificationDetail().get(cnt).getState()
											.getId());

					qualification.setCollegeId(
							hpRegistrationUpdateRequestTO.getQualificationDetail().get(cnt).getCollege().getId() == null
									? null
									: hpRegistrationUpdateRequestTO.getQualificationDetail().get(cnt).getCollege()
											.getId());

					qualification.setUniversityId(hpRegistrationUpdateRequestTO.getQualificationDetail().get(cnt)
							.getUniversity().getId() == null ? null
									: hpRegistrationUpdateRequestTO.getQualificationDetail().get(cnt).getUniversity()
											.getId());

					qualification.setCourseId(
							hpRegistrationUpdateRequestTO.getQualificationDetail().get(cnt).getCourse().getId() == null
									? null
									: hpRegistrationUpdateRequestTO.getQualificationDetail().get(cnt).getCourse()
											.getId());

					qualification.setQualificationYear(
							hpRegistrationUpdateRequestTO.getQualificationDetail().get(cnt).getQualificationYear());
					qualification.setQualificationMonth(
							hpRegistrationUpdateRequestTO.getQualificationDetail().get(cnt).getQualificationMonth());
					qualification.setIsNameChange(
							hpRegistrationUpdateRequestTO.getQualificationDetail().get(cnt).getIsNameChange());
			...
			List<CustomQualificationDetails> customQualificationDetails = iCustomQualificationDetailRepository
					.getQualificationDetailsByHpProfileId(hpProfileId);

			if (customQualificationDetails.size() == 0) {

				List<CustomQualificationDetails> customQualificationDetailsList = new ArrayList<CustomQualificationDetails>();
				for (QualificationDetailRequestTO newCustomQualification : newqualificationDetailTOList) {
					...
					// customQualification.setRegistrationDetails(registrationData);

					HpProfile hpProfile = iHpProfileRepository.findById(hpProfileId).orElse(null);
					customQualification.setHpProfile(hpProfile);

					customQualification.setIsVerified(newCustomQualification.getIsVerified());
					customQualification.setEndDate(null);
					customQualification.setCertificate(null);
					customQualification.setName(null);
					customQualification.setStartDate(null);
					customQualification.setSystemOfMedicine(null);
					customQualificationDetailsList.add(customQualification);
				...
				Integer cnt = 0;
				List<CustomQualificationDetails> customQualificationDetailsList = new ArrayList<CustomQualificationDetails>();
				for (CustomQualificationDetails editCustomQualification : customQualificationDetails) {

					editCustomQualification.setCountry(hpRegistrationUpdateRequestTO.getQualificationDetail().get(cnt)
							.getCountry().getName() == null ? null
									: hpRegistrationUpdateRequestTO.getQualificationDetail().get(cnt).getCountry()
											.getName());

					editCustomQualification.setState(
							hpRegistrationUpdateRequestTO.getQualificationDetail().get(cnt).getState().getName() == null
									? null
									: hpRegistrationUpdateRequestTO.getQualificationDetail().get(cnt).getState()
											.getName());

					editCustomQualification.setCollege(hpRegistrationUpdateRequestTO.getQualificationDetail().get(cnt)
							.getCollege().getName() == null ? null
									: hpRegistrationUpdateRequestTO.getQualificationDetail().get(cnt).getCollege()
											.getName());

					editCustomQualification.setUniversity(hpRegistrationUpdateRequestTO.getQualificationDetail()
							.get(cnt).getUniversity().getName() == null ? null
									: hpRegistrationUpdateRequestTO.getQualificationDetail().get(cnt).getUniversity()
											.getName());

					editCustomQualification.setCourse(hpRegistrationUpdateRequestTO.getQualificationDetail().get(cnt)
							.getCourse().getName() == null ? null
									: hpRegistrationUpdateRequestTO.getQualificationDetail().get(cnt).getCourse()
											.getName());

					editCustomQualification.setQualificationYear(
							hpRegistrationUpdateRequestTO.getQualificationDetail().get(cnt).getQualificationYear());
					editCustomQualification.setQualificationMonth(
							hpRegistrationUpdateRequestTO.getQualificationDetail().get(cnt).getQualificationMonth());
					editCustomQualification.setIsNameChange(
							hpRegistrationUpdateRequestTO.getQualificationDetail().get(cnt).getIsNameChange());

					cnt += 1;
					customQualificationDetailsList.add(editCustomQualification);
				}

				iCustomQualificationDetailRepository.saveAll(customQualificationDetailsList);

			}
			HpNbeDetails hpNbeDetails = hpNbeDetailsRepository.findByRequestId(registrationData.getRequestId());
			if (hpNbeDetails == null) {

				HpNbeDetails addHpNbeDetails = new HpNbeDetails();
				addHpNbeDetails.setMarksObtained(hpRegistrationUpdateRequestTO.getHpNbeDetails().getMarksObtained());
				addHpNbeDetails.setMonthOfPassing(hpRegistrationUpdateRequestTO.getHpNbeDetails().getMonthOfPassing());
				addHpNbeDetails.setRollNo(hpRegistrationUpdateRequestTO.getHpNbeDetails().getRollNo());
				addHpNbeDetails.setUserResult(hpRegistrationUpdateRequestTO.getHpNbeDetails().getUserResult());
				addHpNbeDetails.setYearOfPassing(hpRegistrationUpdateRequestTO.getHpNbeDetails().getYearOfPassing());
				addHpNbeDetails.setCreatedAt(Timestamp.valueOf(LocalDateTime.now()));
				addHpNbeDetails.setUpdatedAt(Timestamp.valueOf(LocalDateTime.now()));
				addHpNbeDetails.setRequestId(requestId);
				hpNbeDetailsRepository.save(addHpNbeDetails);
				;
			} else {
		...
		HpProfileUpdateResponseTO hpProfileUpdateResponseTO = new HpProfileUpdateResponseTO(204,
				"Registration Added/Updated Successfully!!", hpProfileId, requestId);
		return hpProfileUpdateResponseTO;
	}
	...
	public HpProfileUpdateResponseTO updateWorkProfileDetails(BigInteger hpProfileId,
		...
		WorkProfile workProfile = workProfileRepository.getWorkProfileByHpProfileId(hpProfileId);
		WorkProfile adWorkProfile = new WorkProfile();
		if (workProfile == null) {
			WorkProfile addWorkProfile = new WorkProfile();
			addWorkProfile.setBroadSpecialityId(
			...
			adWorkProfile = workProfileRepository.save(addWorkProfile);

		} else {
			workProfile.setBroadSpecialityId(
					hpWorkProfileUpdateRequestTO.getSpecialityDetails().getBroadSpeciality().getId());
			workProfile.setWorkNatureId(hpWorkProfileUpdateRequestTO.getWorkDetails().getWorkNature().getId());
			workProfile.setWorkStatusId(hpWorkProfileUpdateRequestTO.getWorkDetails().getWorkStatus().getId());
			workProfile.setIsUserCurrentlyWorking(
					hpWorkProfileUpdateRequestTO.getWorkDetails().getIsUserCurrentlyWorking());
			workProfile.setFacility(hpWorkProfileUpdateRequestTO.getCurrentWorkDetails().getFacility());
			workProfile.setUrl(hpWorkProfileUpdateRequestTO.getCurrentWorkDetails().getUrl());
			workProfile.setWorkOrganization(hpWorkProfileUpdateRequestTO.getCurrentWorkDetails().getWorkOrganization());

			workProfile.setRequestId(hpWorkProfileUpdateRequestTO.getRequestId());

			workProfileRepository.save(workProfile);

		}
		String requestId = adWorkProfile == null ? hpWorkProfileUpdateRequestTO.getRequestId()
				: adWorkProfile.getRequestId();

		List<SuperSpecialityTO> newSuperSpecialities = hpWorkProfileUpdateRequestTO.getSpecialityDetails()
				.getSuperSpeciality();
		List<SuperSpeciality> superSpecialities = superSpecialityRepository
				.getSuperSpecialityFromHpProfileId(hpProfileId);
		...
		CurrentWorkDetailsTO currentWorkDetailsTO = hpWorkProfileUpdateRequestTO.getCurrentWorkDetails();
...
//	}
	...
	String checkIsNullAndAddSeparator(String string) {
		if (string == null) {
			return "";
		} else {
			return string + ", ";
