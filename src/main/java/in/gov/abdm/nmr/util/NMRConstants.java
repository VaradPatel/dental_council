...
public class NMRConstants {
    ...
    public static final String FETCH_COUNT_QUERY_FOR_SMC = "SELECT application_type_id as applicationTypeId, smc_status as profileStatus, count(*) as count FROM main.dashboard d join main.registration_details rd on rd.hp_profile_id = d.hp_profile_id JOIN main.hp_profile hp on rd.hp_profile_id = hp.id and hp.e_sign_status IN (1,2,3) where application_type_id in (1,3,4,7,8) and rd.state_medical_council_id =:" + SMC_PROFILE_ID + " group by application_type_id, smc_status";
